...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final List<String> _categories = ['Chicken', 'Beef', 'Seafood', 'Vegetarian', 'Pasta', 'Dessert'];
  ...
  Future<void> _fetchMeals() async {
    ...
    try {
      final url = Uri.parse('https://www.themealdb.com/api/json/v1/1/filter.php?c=$_selectedCategory');
      ...
      if (response.statusCode == 200) {
        ...
        setState(() { _errorMsg = 'Gagal memuat data.'; _isLoading = false; });
      }
    } catch (e) {
      setState(() { _errorMsg = 'Koneksi gagal. Periksa internet.'; _isLoading = false; });
    }
  }
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              return ChoiceChip(
                ...
                labelStyle: TextStyle(color: selected ? Colors.white : const Color(0xFF424242), fontSize: 13, fontWeight: FontWeight.w500),
                backgroundColor: const Color(0xFFF5F5F5),
                side: BorderSide(color: selected ? const Color(0xFFFF9800) : const Color(0xFFE0E0E0)),
                onSelected: (_) { setState(() => _selectedCategory = cat); _fetchMeals(); },
              );
            },
          ),
        ),
...
class _MealCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => DetailScreen(mealId: meal.idMeal))),
      child: Container(
            ...
            ClipRRect(
              ...
              child: Image.network(
                ...
                loadingBuilder: (_, child, progress) => progress == null
                    ? child
                    : Container(height: 130, color: const Color(0xFFF5F5F5), child: const Center(child: CircularProgressIndicator(color: Color(0xFFFF9800), strokeWidth: 2))),
